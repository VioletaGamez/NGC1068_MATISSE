...
if(i==9):
   limchisq=20#r1  
   temperatures1=np.linspace(180,400,30)#fine grid final   
   ffs1=np.linspace(0.01,0.7,30)
   logNexts1=np.linspace(-4,-3,30)#

   temperatures2=np.linspace(800,1100,40)#15->chi2:20-24
   ffs2=np.linspace(0.0001,0.01,40)

   #temperatures1=np.linspace(180,500,20)#coarse grid
   #ffs1=np.linspace(0.001,1,20)
   #logNexts1=np.linspace(-6,-2,30)#

   #temperatures2=np.linspace(400,1200,20)#
   #ffs2=np.linspace(0.0001,0.01,20)#.
...
def getrelfn(wls,wldata,ydata):
    rf=np.load('stdrelflxnewN12e-2.npy',allow_pickle=True)#
    fractsurfb1=[]
...
def getrelflm(wls,wldata,ydata):
    rf=np.load('stdrelflxLnewM12e-2.npy',allow_pickle=True)#new M models
    fractsurfb1=[]
...
def getstdrelflm(wls,wldata,ydata):   
    stdrf=np.load('stdrelflxLnewM12e-2.npy',allow_pickle=True)#for 12e new M models
    errrelfluxlm=[]
...
def getstdrelfn(wls,wldata,ydata):
    stdrf=np.load('stdrelflxnewN12e-2.npy',allow_pickle=True)#for 12 ellipses new N
    errrelflux=[]
    for i in range(9):
...
def fp(dfn,label,grav):   #leg=fp(dfn,'no')
    ...
    np.save('final',en+run+'E12olivinesmallC'+np.str(Cper)+'minmaxf.npy',[minfwl,maxfwl])
